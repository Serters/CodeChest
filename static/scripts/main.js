...
async function draw_main(where) {
	...
	if (snippet_list_default.snippets.length > 0) {
	...
	let dsc;
	if (snippet_list_default.snippets.length > 0) {
		dsc = snippet_list_default.get_snippet_by_id(active_snippet).full_desc;
	} else {
		dsc = "Empty";
	}
	let full_description = new snippet_description(dsc);
	...
	confirm_delete_button.addEventListener("click", delete_snippet);

	delete_button.addEventListener("click", () => {
		actions_div.removeChild(delete_button);
		actions_div.appendChild(confirm_delete_button);
	});
	//#endregion - - - - - RIGHT - - - - -
	...
	async function delete_snippet() {
		try {
			...
			update_active_snippet(
				newest_snippet.full_desc,
				newest_snippet.snippet_id
			);
		} catch (error) {
